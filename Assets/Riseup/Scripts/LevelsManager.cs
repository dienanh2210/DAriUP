...
public class LevelsManager : MonoBehaviour {
	...
	private void Start () {
        ...
        HightLevel.text = PlayerPrefs.GetInt("HightLevel1", 0).ToString();
	...
	public void SpawnLevel () {
		...
		LevelText.text = currentLevel.ToString () + "/80";
		currentLevel++;

        if (currentLevel > PlayerPrefs.GetInt("HightLevel1", 0))
        {
            PlayerPrefs.SetInt("HightLevel1", currentLevel);
            HightLevel.text = currentLevel.ToString() + "/80";
